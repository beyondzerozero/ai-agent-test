...
class ReflectiveAgentState(BaseModel):
    query: str = Field(..., description="사용자가 처음 입력한 쿼리")
    ...
    reflection_ids: Annotated[list[str], operator.add] = Field(
        default_factory=list, description="리플렉션 결과 ID 목록"
    )
    final_output: str = Field(default="", description="최종 출력 결과")
    retry_count: int = Field(default=0, description="작업 재시도 횟수")
...
class ReflectiveGoalCreator:
    ...
    def run(self, query: str) -> str:
        ...
        query = f"{query}\n\n목표 설정 시 다음과 같은 과거 회고록을 고려해야 한다:\n{reflection_text}"
...
class ReflectiveResponseOptimizer:
    ...
    def run(self, query: str) -> str:
        ...
        query = f"{query}\n\n응답 최적화를 위해 다음과 같은 과거 회귀를 고려해야 한다:\n{reflection_text}"
...
class QueryDecomposer:
    ...
    def run(self, query: str) -> DecomposedTasks:
        ...
        prompt = ChatPromptTemplate.from_template(
            ...
            "4. 작업은 한국어로 출력한다.\n"
            "5. 작업을 작성할 때 다음과 같은 과거 회귀를 고려해야 합니다:\n{reflections}\n\n"
            "목표: {query}"
        )
        ...
        result = agent.invoke(
            {
                "messages": [
                    (
                        ...
                        "3. 가능한 한 구체적인 사실과 데이터를 제공해야 한다.\n"
                        "4. 발견 사항을 명확하게 요약해야 한다.\n"
                        f"5. 다음과 같은 과거를 되돌아보는 것을 고려해야 한다:\n{reflection_text}\n",
                    )
                ]
            }
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ReflectiveAgent를 사용하여 작업 수행（Self-reflection）"
    )
